...
var proto = {
    ...
    run: function run(context, states, callback) {
        ...
        if (key) {
            ...
            try {
                ...
                execution = this._getCurrentExecution(context.key, states);
            ...
            this._getNextState(execution, eventId, function onNextState(err, nextState) {
                if (err) {
                    complete(err);
                    return;
                }
                self._execute(execution, nextState, complete);
            });
            return;
        }
    ...
    _getCurrentExecution: function _getCurrentExecution(executionKey, states) {
    ...
    _getNextState: function _getNextSta(execution, eventId, callback) {
