...
var _proto = {
    ...
    run: function run(context, states, callback) {
        ...
        if (key) {
            ...
            this.getCurrentExecution(context.key, states, function onCurrentExecution(err, exec) {
                if (err) {
                    complete(err);
                    return;
                }
                ...
                execution = exec;
                self.getNextState(exec, eventId, function onNextState(err, nextState) {
                    if (err) {
                        complete(err);
                        return;
                    }

                    self.execute(exec, nextState, complete);
                });
        ...
        this.execute(execution, this.startFlow.startState, complete);
    },

    getCurrentExecution: function getCurrentExecution(executionKey, states, callback) {
    ...
    getNextState: function getNextState(execution, eventId, callback) {
    ...
    execute: function execute(execution, nextState, callback) {
        ...
        switch (state.type) {
            ...
            case 'action':
                ...
                state.impl.onEntry(function onEntryDone(err, eventId) {
                    ...
                    self.execute(execution, event, callback);
                });
                break;
            ...
            case 'subflow':
                ...
                this.execute(execution, flow.startState, callback);
                break;
            ...
            case 'end':
                ...
                flow = this.definition[context.flow];
                state = flow.states[context.state];
                this.execute(execution, state.transitions[state.name], callback);
                break;
...
var proto = Object.create(Object.prototype, {
    run: {value: _proto.run, enumerable: true},
    getCurrentExecution: {value: _proto.getCurrentExecution},
    getNextState: {value: _proto.getNextState},
    execute: {value: _proto.execute}
});
